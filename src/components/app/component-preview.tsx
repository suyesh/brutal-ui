...
import productUtility from "@/data/product-utility"
...
export default function ComponentPreview({
  ...
  example?: string
  type?: "star" | "component" | "block" | "utility"
  ...
  if (type === "star") {
    ...
    const blockData = blocks.find((block) => transformToSlug(block.name) === component)

    if (!blockData) return null

    ExampleComponent = blockData.exampleComponent
  } else if (type === "utility") {
    const utilityData = productUtility.find(
      (block) => transformToSlug(block.name) === component,
    )

    if (!utilityData) return null

    ExampleComponent = utilityData.exampleComponent
  } else {
